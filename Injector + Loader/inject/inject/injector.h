...
/////////////////////////////////
bool Kernel_Injector(LPCWSTR windowname, BYTE* dll_path)
{
	Sleep(100);
	// get dll file
	...
	while (!process_id)
	{
		thread_id = GetWindowThreadProcessId(FindWindow(NULL, xor_w(L"Call of Duty® HQ")), &process_id);
		Sleep(20);
	}
#ifdef DEBUG_PRINT
	...
	if (process_id != 0 && thread_id != 0)
	{
...
#ifdef DEBUG_PRINT
		printf(xor_a(" [+] Relocation Success!\n"));
#endif
		...
		return true;
#ifdef DEBUG_PRINT
		std::cout << GREEN << printf(xor_a(" [+] Injected successfully.\n"));
		cout << endl;
#endif
	}
	else
