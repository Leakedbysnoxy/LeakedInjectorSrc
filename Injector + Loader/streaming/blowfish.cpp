...
BLOWFISH::BLOWFISH(std::string hexKey)
{
	...
	bytes* key = new bytes[hexKey.length() / 2];//
...
BLOWFISH::BLOWFISH(bytes* cipherKey, int keyLength)
{
	IvSet = false;
	SetupKey(cipherKey, keyLength);
}

bytes* BLOWFISH::Encrypt_ECB(bytes* data, int length, int* newlength)
{
	return Crypt_ECB(data, length, newlength, &BLOWFISH::Encrypt_Block, false);
}

bytes* BLOWFISH::Decrypt_ECB(bytes* data, int length, int* newlength)
{
	return Crypt_ECB(data, length, newlength, &BLOWFISH::Decrypt_Block, true);
}

bytes* BLOWFISH::Encrypt_CBC(bytes* data, int length, int* newlength)
{
	return Crypt_CBC(data, length, newlength, &BLOWFISH::Encrypt_Block, false);
}

bytes* BLOWFISH::Decrypt_CBC(bytes* data, int length, int* newlength)
{
...
{
	bytes* binaryData = new bytes[data.length()];
	for (int i = 0; i < data.length(); i++)
		binaryData[i] = data[i];
	int newlen = 0;
	bytes* result = Encrypt_CBC(binaryData, data.length(), &newlen);
	std::string encoded = "";
	for (int i = 0; i < newlen; i++)
		encoded += bytesToHex(result[i]);
	delete[] result;
	delete[] binaryData;
	return encoded;
}
...
std::string BLOWFISH::Decrypt_CBC(std::string data)
{
	...
	bytes* binaryData = new bytes[data.length() / 2];//
	...
	int len = 0;
	bytes* cryptresult = Decrypt_CBC(binaryData, data.length() / 2, &len);
	std::string result = "";
	for (int i = 0; i < len; i++)
...
std::string BLOWFISH::bytesToHex(unsigned char x)
{
...
bytes* BLOWFISH::padData(bytes* data, int length, int* paddedLength, bool decrypt, bool IvSpace = false)
{
	int offset = 0;
	int dataoffset = 0;
	if (decrypt)
	...
	bytes* pData = new bytes[*paddedLength];
...
int BLOWFISH::findPaddingEnd(bytes* data, int length)
{
	int i = length;
...
bytes* BLOWFISH::Crypt_ECB(bytes* data, int length, int* newlength, void (BLOWFISH::* CryptBlock)(bytes*, int), bool decrypt)
{
	bytes* pData;
...
bytes* BLOWFISH::Crypt_CBC(bytes* data, int length, int* newlength, void (BLOWFISH::* CryptBlock)(bytes*, int), bool decrypt)
{
	bytes* pData;
	if (!decrypt && !IvSet)
		SetRandomIV();
	...
	if (!decrypt)
	{
		//padData leaves an 8 bytes block at the beggining so we can save the IV
		for (int i = 0; i < 8; i++)
			pData[i] = IV[i];
	}
	else
	...
	if (decrypt) //if we are decrypting, we have to find where the data ends, and remove the IV
	{
		*newlength = findPaddingEnd(pData, *newlength) - 8;
		bytes* noIV = new bytes[*newlength];
...
{
#ifdef _WIN32
	//WIN32 CSPRNG thanks to: http://www.tomhandal.com/DevBlog/2010/03/17/cryptographically-random-bytess-in-microsoft-windows/
...
void BLOWFISH::SetIV(bytes* newIV)
{
	IvSet = true;
	for (int i = 0; i < 8; i++)
		IV[i] = newIV[i];
...
bytes* BLOWFISH::GetIV()
{
	bytes* returnIV = new bytes[8];
	for (int i = 0; i < 8; i++)
		returnIV[i] = IV[i];
	return returnIV;
}

void BLOWFISH::Encrypt_Block(bytes* block, int offset)
{
...
void BLOWFISH::Decrypt_Block(bytes* block, int offset)
{
	setblock(block, offset);
	decipher();
	getblock(block, offset);
...
void BLOWFISH::setblock(bytes* block, int offset)
{
...
void BLOWFISH::getblock(bytes* block, int offset)
{
	//TODO: CHECK ENDIANNESS
...
void BLOWFISH::SetupKey(bytes* cipherKey, int length)
{
	if (length > 56)
	{
		throw 56;
